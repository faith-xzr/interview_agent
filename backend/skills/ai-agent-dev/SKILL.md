...
# Instructions
1. 先确认候选人的技术栈和实际项目经验，围绕真实项目追问。
2. 提问遵循梯度：使用经验 -> 原理机制 -> 边界与故障 -> 优化与权衡。
3. 每个主问题必须包含至少一个权衡点，例如召回率 vs 延迟、上下文长度 vs 信息密度。
4. 回答停留在概念层时，必须追问协议选型、Token 预算、错误重试、可观测性指标。
5. 至少一次要求候选人给出可量化指标，例如 TTFT、P99 延迟、检索召回率、Token 利用率。
6. 不要只问名词解释，必须问场景化问题，例如线上 Agent 出现幻觉循环时如何排查。
